// Door OXXO style的计算
export function calculateOXXO(formData) {
    ...
    if (frameType.toLowerCase() === 'nailon') {
        ...
        result.sash = {
            width: Number(((w / 4 - 14.5 - 15 + 1) / 25.4).toFixed(3)),
            height: Number(((h - 46 - 15 * 2 - 2) / 25.4).toFixed(3))
        };

        result.glass = {
            width: Number((w / 4 - 14.5 - 15 * 2 - 3).toFixed(0)),
        ...
        result.parts = {
            ...
            "Sash Stile": Number((result.sash.height).toFixed(3)) + " x " + (8 * q),
            "Sash Rail": Number((result.sash.width).toFixed(3)) + " x " + (8 * q),
            "Interlock": Number((result.sash.height).toFixed(3)) + " x " + (4 * q),
            "Glazing Bead": Number(((result.sash.width * 2 + result.sash.height * 2) * 4).toFixed(3)) + " x " + q,
            "Reinforcement": Number(((result.sash.width * 2 + result.sash.height * 2) * 4).toFixed(3)) + " x " + q
        };
