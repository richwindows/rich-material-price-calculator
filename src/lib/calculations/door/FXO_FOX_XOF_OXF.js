// Door FXO/FOX/XOF/OXF style的计算
export function calculateFXO_FOX_XOF_OXF(formData) {
    const width = parseFloat(formData.width || 0);
    const height = parseFloat(formData.height || 0);
    const fixHeight = parseFloat(formData.fixHeight || 0);
    const w = width * 25.4;  // Convert to mm
    const h = height * 25.4;
    const fh = fixHeight * 25.4;
    ...
    if (frameType.toLowerCase() === 'nailon') {
        ...
        result.sash = {
            ...
            height: Number(((h - fh - 46 - 15 * 2 - 2) / 25.4).toFixed(3))
        };

        result.fixedSash = {
            width: Number(((w / 2 - 14.5 - 15 + 1) / 25.4).toFixed(3)),
            height: Number(((fh - 46 - 15 * 2 - 2) / 25.4).toFixed(3))
        };

        result.glass = {
            width: Number((w / 2 - 14.5 - 15 * 2 - 3).toFixed(0)),
            height: Number((h - fh - 46 - 15 * 2 - 3 * 2).toFixed(0))
        };

        result.fixedGlass = {
            width: Number((w / 2 - 14.5 - 15 * 2 - 3).toFixed(0)),
            height: Number((fh - 46 - 15 * 2 - 3 * 2).toFixed(0))
        };
        ...
        result.parts = {
            ...
            "Fixed Sash Stile": Number((result.fixedSash.height).toFixed(3)) + " x " + (4 * q),
            "Fixed Sash Rail": Number((result.fixedSash.width).toFixed(3)) + " x " + (4 * q),
            "Interlock": Number((result.sash.height).toFixed(3)) + " x " + (2 * q),
            "Glazing Bead": Number(((result.sash.width * 2 + result.sash.height * 2 + result.fixedSash.width * 2 + result.fixedSash.height * 2) * 2).toFixed(3)) + " x " + q,
            "Reinforcement": Number(((result.sash.width * 2 + result.sash.height * 2 + result.fixedSash.width * 2 + result.fixedSash.height * 2) * 2).toFixed(3)) + " x " + q
        };
