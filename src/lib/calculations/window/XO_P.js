// XO-P style window calculations
export function calculateXO_P(formData) {
  const { 
    width, 
    height, 
    fixedHeight,
  ...
  if (frameType === 'nailon') {
    ...
    // Sash calculations
    const sashw = roundToThree((w / 2 - 14.5 - 15 + 1) / 25.4);
    const sashh = roundToThree((h - fh - 6 - 46 - 15 - 2 - 1) / 25.4);
    result.sash = {
      width: sashw,
      height: sashh,
      quantity: {
    ...
    const screenh = Math.round(h - fh - 6 - 87 - 15 - 4);
    result.screen = {
      width: screenw,
      height: screenh,
      quantity: {
    ...
    // Parts calculations
    const mullion = roundToThree((h - fh - 6 - 36 - 15) / 25.4);
    const mullionA = roundToOne((h - fh - 6 - 36 - 15) / 25.4 - 2);
    const handleA = Math.round((h - fh - 6 - 46 - 15) / 25.4 / 2 + 4);
    const track = roundToOne((w - 14 * 2 - 15 * 2 - 3 - 20) / 25.4);
    ...
    // Glass calculations
    const sashglassw = w / 2 - 77 - 15;
    const sashglassh = h - fh - 6 - 109 - 15 - 3 - 2;
    const fixedglassw = w / 2 - 44 - 15;
    const fixedglassh = h - fh - 6 - 47 - 15 - 2;
    const fixedglass2w = w - 20.5 * 2 - 3 * 2 - 15 * 2;
    const fixedglass2h = fh - 6 - 20.5 * 2 - 3 * 2 - 15 - 2;
    ...
    if (gridType) {
      ...
      if (gridType === 'Standard') {
        ...
        result.grid.sash.widthHole = 102;
        result.grid.sash.heightHole = 70;
    ...
    switch (glassType) {
      ...
      // Similar cases for other glass types...
      // Add cases for Clear/Lowe3, OBS/Clear, OBS/Lowe2, OBS/Lowe3
      // And their tempered variants
    }
  } else {
    ...
    // Similar calculations for other frame types...
    // Add sash, screen, parts, glass calculations
    // Add grid calculations and glass orders processing
  }
