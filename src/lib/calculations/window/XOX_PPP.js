// XOX-PPP style window calculations
export function calculateXOX_PPP(formData) {
...
function calculateGlassOrders(formData, glassCalc) {
  ...
  const processGlassType = (type, isTempered = false) => {
    ...
    switch(type) {
      case "Clear/Clear":
        addGlassOrder("--01", 4 * q, "clear", treatment, glassCalc.sash);
        ...
        if (topBottom === "1" || isTempered) {
          ...
          addGlassOrder("--04", 2 * q, "clear", "T", glassCalc.fixed3);
          addOrder("--03", 4 * q, "Clear", glassCalc.fixed2);
          addOrder("--04", 2 * q, "Clear", glassCalc.fixed3);
        } else {
      ...
      case "OBS/Lowe3":
        ...
        if (type === "OBS/Lowe2" || type === "OBS/Lowe3") {
          addGlassOrder("--01", 2 * q, glassType2, treatment, glassCalc.sash);
          addGlassOrder("--01", 2 * q, glassType1, treatment, glassCalc.sash);
          addGlassOrder("--02", 1 * q, glassType2, treatment, glassCalc.fixed);
          addGlassOrder("--02", 1 * q, glassType1, treatment, glassCalc.fixed);

          if (topBottom === "1" || isTempered) {
            addGlassOrder("--03", 2 * q, glassType2, "T", glassCalc.fixed2);
            addGlassOrder("--03", 2 * q, glassType1, "T", glassCalc.fixed2);
            addGlassOrder("--04", 1 * q, glassType2, "T", glassCalc.fixed3);
            addGlassOrder("--04", 1 * q, glassType1, "T", glassCalc.fixed3);
            addOrder("--03", 2 * q, orderType2, glassCalc.fixed2);
            addOrder("--03", 2 * q, orderType1, glassCalc.fixed2);
            addOrder("--04", 1 * q, orderType2, glassCalc.fixed3);
            addOrder("--04", 1 * q, orderType1, glassCalc.fixed3);
          } else {
